...
def load_and_match_image(template_folder, image_num, devices=None):
    """加载屏幕和模板图像，并使用OpenCV进行匹配。"""
    adb.adb_image(devices)
    screen_path = f'./data/img/{devices}screen.png'
...
def find_and_act_on_image(template_folder, image_num, cooldown, action, max_attempts=None,  devices=None):
    ...
    while max_attempts is None or attempts < max_attempts:
        attempts += 1
        width, height, locations = load_and_match_image(template_folder, image_num, devices)
